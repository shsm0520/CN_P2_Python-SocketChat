...
what is setsockopt?
    ...
    so reuseaddress
    (level, optname, value)
    i think something about reusing addresses for sockets that are set to waiting and then yeah
"""
...
def client_test_thread_main():
    """
    making sure that the server and client can connect
    """

    while True:
        clientSocket.send(input("please pass me a greeting\n.").encode()) # send string to server

def client_user_connect(socket: socket):
    """
    testing the first iteration of user_connect()
    """

    # print message
    username = input(f"{socket.recv(1024).decode()}\n>")
...
if __name__ == "__main__":
    ...
    clientSocket.connect(('localhost', 6789)) # connect socket to a port

    client_user_connect(clientSocket)
    clientSocket.close()